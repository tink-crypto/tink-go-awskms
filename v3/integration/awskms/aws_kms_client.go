...
// KMSAPI is a subset of the methods in the interface of *kms.Client that we use in this package.
...
func (o option) set(ctx context.Context, a *awsClient) error { return o(ctx, a) }
...
func WithCredentialPath(credentialPath string) ClientOption {
	return option(func(ctx context.Context, a *awsClient) error {
		if a.kms != nil {
			return errors.New("WithCredentialPath option cannot be used, KMS client already set")
		}

		cfg, err := getConfigFromCredentialPath(ctx, a.keyURIPrefix, credentialPath)
		if err != nil {
			return err
		}
		a.kms = kms.NewFromConfig(cfg)
		return nil
	})
}
...
//
//	awskms.NewClientWithOptions(ctx, uriPrefix, awskms.WithCredentialPath(credentialPath))
func NewClientWithCredentials(ctx context.Context, uriPrefix string, credentialPath string) (registry.KMSClient, error) {
	return NewClientWithOptions(ctx, uriPrefix, WithCredentialPath(credentialPath), WithEncryptionContextName(LegacyAdditionalData))
}
...
//
//	awskms.NewClientWithOptions(ctx, uriPrefix, awskms.WithKMS(kms))
func NewClientWithKMS(ctx context.Context, uriPrefix string, kms KMSAPI) (registry.KMSClient, error) {
...
func getConfigFromCredentialPath(ctx context.Context, uriPrefix string, credentialPath string) (aws.Config, error) {
	r, err := getRegion(uriPrefix)
	if err != nil {
	...
	if len(credentialPath) == 0 {
		return aws.Config{}, errCred
	}

	// Attempt to load as static credentials from CSV
	creds, err := extractCredsCSV(credentialPath)
	switch err {
	case nil:
	...
	case errBadFile, errCredCSV:
		return aws.Config{}, err
	default:
		// Fallback to load as shared credentials from INI file
		return config.LoadDefaultConfig(ctx,
			config.WithRegion(r),
			config.WithSharedConfigProfile("default"),
			config.WithSharedConfigFiles([]string{credentialPath}),
		)
	}
}
