...
func TestNewClientWithOptions_URIPrefix(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := NewClientWithOptions(t.Context(), test.uriPrefix)
			...
			_, err = NewClient(t.Context(), test.uriPrefix)
			if test.valid && err != nil {
			...
			_, err = NewClientWithCredentials(t.Context(), test.uriPrefix, credFile)
			if test.valid && err != nil {
			...
			_, err = NewClientWithKMS(t.Context(), test.uriPrefix, fakekms)
			if test.valid && err != nil {
...
func TestNewClientWithOptions_WithCredentialPath(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := NewClientWithOptions(t.Context(), uriPrefix, WithCredentialPath(test.credFile))
			if test.valid && err != nil {
				t.Errorf("NewClientWithOptions(t.Context(), uriPrefix, WithCredentialPath(%q)) err = %v, want nil", test.credFile, err)
			}
			if !test.valid && err == nil {
				t.Errorf("NewClientWithOptions(t.Context(), uriPrefix, WithCredentialPath(%q)) err = nil, want error", test.credFile)
			}

			// Test deprecated factory function.
			_, err = NewClientWithCredentials(t.Context(), uriPrefix, test.credFile)
			if test.valid && err != nil {
...
func TestNewClientWithOptions_RepeatedWithKMSFails(t *testing.T) {
	...
	_, err = NewClientWithOptions(t.Context(), "aws-kms://", WithKMS(fakekms), WithKMS(fakekms))
	if err == nil {
		t.Fatalf("NewClientWithOptions(t.Context(), _, WithKMS(_), WithKMS(_)) err = nil, want error")
	}
}

func TestNewClientWithOptions_RepeatedWithEncryptionContextNameFails(t *testing.T) {
	_, err := NewClientWithOptions(t.Context(), "aws-kms://", WithEncryptionContextName(LegacyAdditionalData), WithEncryptionContextName(AssociatedData))
	if err == nil {
		t.Fatalf("NewClientWithOptions(t.Context(), _, WithEncryptionContextName(_), WithEncryptionContextName(_)) err = nil, want error")
	}
}
...
func TestSupported(t *testing.T) {
	...
	client, err := NewClientWithOptions(t.Context(), uriPrefix)
	if err != nil {
...
func TestGetAEADEncryptDecrypt(t *testing.T) {
	...
	client, err := NewClientWithOptions(t.Context(), "aws-kms://", WithKMS(fakekms))
	if err != nil {
		t.Fatalf("NewClientWithOptions() failed: %v", err)
	}
...
func TestUsesAdditionalDataAsContextName(t *testing.T) {
	...
	client, err := NewClientWithKMS(t.Context(), "aws-kms://", fakekms)
	if err != nil {
	...
	encryptionContext := map[string]string{"additionalData": hexAD}
...
func TestEncryptionContextName(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.wantContextName, func(t *testing.T) {
			client, err := NewClientWithOptions(t.Context(), "aws-kms://", WithKMS(fakekms), WithEncryptionContextName(test.contextName))
			if err != nil {
				t.Fatalf("NewClientWithOptions() failed: %v", err)
			}
			...
			hexAD := hex.EncodeToString(associatedData)
			encryptionContext := map[string]string{test.wantContextName: hexAD}
			decRequest := &kms.DecryptInput{
				KeyId:             aws.String(keyARN),
				CiphertextBlob:    ciphertext,
				EncryptionContext: encryptionContext,
...
func TestEncryptionContextName_defaultEncryptionContextName(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			client: func(t *testing.T) registry.KMSClient {
				t.Helper()
				c, err := NewClientWithOptions(t.Context(), keyURI, WithKMS(fakekms))
				if err != nil {
					t.Fatalf("NewClientWithOptions() failed: %v", err)
				}
				return c
		...
		// Test deprecated factory function.
		{
			...
			client: func(t *testing.T) registry.KMSClient {
				t.Helper()
				c, err := NewClientWithKMS(t.Context(), keyURI, fakekms)
				if err != nil {
					t.Fatalf("NewClientWithKMS() failed: %v", err)
				}
				return c
